// A "backwards" 1-dimensional array-based implementation of the `matrix.h`
// interface as recommended by Joseph Manning.
...
matrix* matrix_new(int num_rows, int num_cols) {
	...
	m->rows = malloc(sizeof(double) * num_rows * num_cols);
	m->num_rows = num_rows;
	m->num_cols = num_cols;
