...
fields = ['func', 'cc', 'opt', 'size']

parser = argparse.ArgumentParser(description='Plot the timing of a function')
parser.add_argument('-f', '--func', type=strs, default=glob_match("src/time_*.c"), help="matrix function to time")
...
for i in xrange(len(cs)):
    ...
    fname = ("build/dat/time_" + c['func'] + '_%s_'
            + '_'.join(c[f] for f in fields if f != 'func')
            + '_' + str(args['num_points']) + '.txt')

    lines = {}
    for impl in impls:
        env_vars = [('IMPL', impl)] + [(f.upper(), c[f]) for f in fields if f != 'func'] + [('NUM_POINTS', args['num_points'])]
